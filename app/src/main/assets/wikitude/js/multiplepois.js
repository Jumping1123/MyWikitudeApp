...
var World = {
	...
	loadPoisFromJsonData: function loadPoisFromJsonDataFn(poiData) {
		...
		World.markerDrawable_idle = new AR.ImageResource("assets/marker_idle.png"); //ARオブジェクト非選択時の画像
		World.markerDrawable_selected = new AR.ImageResource("assets/marker_selected.png"); //ARオブジェクト選択時の画像
	...
	requestDataFromLocal: function requestDataFromLocalFn(centerPointLatitude, centerPointLongitude) {
		...
		for (var i = 0; i < poisToCreate; i++) {
			poiData.push({
				"id": (i + 1),
				"longitude": (centerPointLongitude + (Math.random() / 5 - 0.1)), //現在位置centerPointLongitudeを基準にランダムに設定
				"latitude": (centerPointLatitude + (Math.random() / 5 - 0.1)), //現在位置centerPointLatitudeを基準にランダムに設定
